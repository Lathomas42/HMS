...
class Neuron(object):
    def __init__(self, skel, conn=None):
        if type(skel) == file:
            skeleton = json.load(skel)
        elif (type(skel) == int) & (conn is not None):
            # need connection in here
            skeleton = conn.skeleton(skel)
        elif (type(skel) == int) & (conn is None):
            print "Passing Skeleton Type int requires a Connection Object"
            return
        elif type(skel) == dict:
            skeleton = skel
        else:
            print "Not Valid Argument Type"
            return
        ...
        for cid in self.dedges:
            for pid in self.dedges[cid]:
                ...
                if self.dedges[cid][pid]['type'] == 'neurite':
                    ...
                    print("Found edge in {} {}{} with"
                          "unknown type {}".format(
                              self.name, cid, pid,
                              self.dedges[cid][pid]['type']))
        ...
        for vk in self.vertices:
            ...
            if 'soma' in v['labels']:
                if self.soma is not None:
                    raise ValueError(
                        "Neuron {} has two somas: {} {}".format(
                            self.name, vk, self.soma))
                self.soma = vk
            if v['type'] == 'connector':
        ...
        for axa in self.axons.keys():
            for axb in self.axons.keys():
                if (axa == axb) or \
                        (axb not in self.axons) or \
                        (axa not in self.axons):
                    continue
        ...
        if not len(sg):
            raise ValueError(
                "unable to find root in {}, graph "
                "contains no nodes [{} vertices]".format(
